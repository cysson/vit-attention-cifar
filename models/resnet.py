...
from models.attention import SELayer, CBAM, ECALayer
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, attention=None):
    ...
    def _make_layer(self, block, planes, num_blocks, stride, attention):
        strides = [stride] + [1] * (num_blocks - 1)
        layers = []
        for stride in strides:
            layers.append(block(self.in_planes, planes, stride, attention))
...
def ResNet50(num_classes=10, **kwargs):
    return ResNet(BasicBlock, [3, 4, 6, 3], num_classes=num_classes, **kwargs)  # 简化版本
